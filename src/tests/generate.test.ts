...
describe("generateMcpServerEnvExample", () => {
  ...
  it("documents credential sections", () => {
    const env = generateMcpServerEnvExample();
    assert.ok(
      env.includes("Your Rocket.Chat credentials") ||
        env.includes("ROCKETCHAT_USER"),
      "should document user credentials",
    );
    assert.ok(
      env.includes("token auth") || env.includes("ROCKETCHAT_AUTH_TOKEN"),
      "should document token auth option",
    );
  });
});
