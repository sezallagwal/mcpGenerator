...
const STEP_FIELD_ACCESS_RE = /steps\.(\w+)\.result\.(\w+)/g;
    ...
    for (const tmpl of templates) {
      ...
      if (paramProps.size > 0) {
        for (const match of tmpl.matchAll(PARAM_REF_RE)) {
          const field = match[1];
          if (!paramProps.has(field)) {
            throw new ComposerError(
              `Step "${step.id}" references "params.${field}" but "${field}" is not in the workflow params schema. Available: ${[...paramProps].join(", ") || "(none)"}`,
            );
          }
        }
      }
        ...
        if (outputType === "string") {
          throw new ComposerError(
            `Step "${step.id}" accesses ".${field}" on step "${refStepId}" (${refStep.config.type}), ` +
              ...
              `(2) Add a transform step after "${refStepId}" with expression 'JSON.parse(steps.${refStepId}.result)', ` +
              `then (3) reference 'steps.<transform_step>.result.${field}' instead. ` +
              `Alternatively, evaluate the raw string directly (e.g. 'steps.${refStepId}.result.includes("...")').`,
          );
        }
        if (outputType === "boolean") {
          throw new ComposerError(
            `Step "${step.id}" accesses ".${field}" on step "${refStepId}" (conditional), ` +
              ...
              `Use the boolean value directly: 'steps.${refStepId}.result === true'.`,
          );
        }
      }
    }
...
function normalizeEventParamShorthand(
  ...
  const templateRewriters = paramProps.map((name) => ({
    name,
    re: new RegExp(`\\{\\{(?!params\\.)${name}\\.`, "g"),
    replacement: `{{params.${name}.`,
  }));

  const jsRewriters = paramProps.map((name) => ({
    name,
  ...
  for (const parentName of paramProps) {
    const parentSchema = params.properties![parentName] as JSONSchema7 | undefined;
  ...
  function rewriteTemplate(
    ...
    for (const rule of subFieldRewriters) {
      const rewritten = result.replace(rule.templateRe, rule.templateReplacement);
      if (rewritten !== result) {
        warnings.push({
          stepId,
          code: "EVENT_PARAM_REWRITTEN",
...
function normalizeTemplateFields(steps: ComposeStepInput[]): ComposerWarning[] {
  ...
  function normalizeString(
    ...
    for (const asVar of asVars) {
      ...
      const rewritten = result.replace(asRefRe, `{{steps.${asVar}.result.$1}}`);
      if (rewritten !== result) {
        warnings.push({
          stepId,
  ...
  for (const step of steps) {
    ...
    switch (cfg.type) {
      ...
      // transform and conditional use raw JS — do NOT normalize
    }
  }
...
export function composeWorkflowDefinition(
  ...
  const { name, description, params, steps, persistence } = input;
